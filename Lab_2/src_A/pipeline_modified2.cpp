...
void pipe_cycle(Pipeline *p)
{
    ...
    pipe_print_state(p);
    std::cout << "valid: " << p->pipe_latch[IF_LATCH][0].valid << " " << p->pipe_latch[ID_LATCH][0].valid << " " << p->pipe_latch[EX_LATCH][0].valid << " "<< p->pipe_latch[MA_LATCH][0].valid << std::endl;
    std::cout << "stall: " << p->pipe_latch[IF_LATCH][0].stall << " " << p->pipe_latch[ID_LATCH][0].stall << " " << p->pipe_latch[EX_LATCH][0].stall << " "<< p->pipe_latch[MA_LATCH][0].stall << std::endl;
}
...
void pipe_cycle_ID(Pipeline *p)
{
    for (unsigned int i = 0; i < PIPE_WIDTH; i++)
    {
        ...
        if (p->pipe_latch[ID_LATCH][i].op_id > 0) {
        ...
        for (unsigned int ii = 0; ii < PIPE_WIDTH; ii++) {
            ...
            if (p->pipe_latch[EX_LATCH][ii].trace_rec.dest_reg == p->pipe_latch[IF_LATCH][i].trace_rec.src1_reg && p->pipe_latch[IF_LATCH][i].trace_rec.src1_needed && p->pipe_latch[EX_LATCH][i].valid) {
                //p->pipe_latch[IF_LATCH][i].stall = true;
                stall_src1_ex = true;
            }

            //Check for a colission between the EX dest reg and the second src reg for ID
            if (p->pipe_latch[EX_LATCH][ii].trace_rec.dest_reg == p->pipe_latch[IF_LATCH][i].trace_rec.src2_reg && p->pipe_latch[IF_LATCH][i].trace_rec.src2_needed && p->pipe_latch[EX_LATCH][i].valid) {
                //p->pipe_latch[IF_LATCH][i].stall = true;
                stall_src2_ex = true;
            }

            //Check for a colission between the EX cc_write reg and the cc_read ID
            if (p->pipe_latch[EX_LATCH][ii].trace_rec.cc_write == p->pipe_latch[IF_LATCH][i].trace_rec.cc_read && p->pipe_latch[EX_LATCH][i].valid) {
                //p->pipe_latch[IF_LATCH][i].stall = true;
                stall_cc_ex = true;
            }

            //Check for a colission between the MA dest reg and the first src reg for ID
            if (p->pipe_latch[MA_LATCH][ii].trace_rec.dest_reg == p->pipe_latch[IF_LATCH][i].trace_rec.src1_reg && p->pipe_latch[IF_LATCH][i].trace_rec.src1_needed && p->pipe_latch[MA_LATCH][i].valid) {
                //p->pipe_latch[IF_LATCH][i].stall = true;
                stall_src1_ma = true;
            }

            //Check for a colission between the MA dest reg and the second src reg for ID
            if (p->pipe_latch[MA_LATCH][ii].trace_rec.dest_reg == p->pipe_latch[IF_LATCH][i].trace_rec.src2_reg && p->pipe_latch[IF_LATCH][i].trace_rec.src2_needed && p->pipe_latch[MA_LATCH][i].valid) {
                //p->pipe_latch[IF_LATCH][i].stall = true;
                stall_src2_ma = true;
            }

            //Check for a colission between the MA cc_write reg and the cc_read ID
            if (p->pipe_latch[MA_LATCH][ii].trace_rec.cc_write == p->pipe_latch[IF_LATCH][i].trace_rec.cc_read && p->pipe_latch[MA_LATCH][i].valid) {
                //p->pipe_latch[IF_LATCH][i].stall = true;
                stall_cc_ma = true;
            }
        }
        ...
        if (stall_cc_ma) {
            std::cout << "MA stall CC" << std::endl;
            //std::cout << p->pipe_latch[ID_LATCH][i].trace_rec.cc_read << " " << p->pipe_latch[MA_LATCH][i].trace_rec.cc_write << std::endl;
        }

        if (stall_age) {
            std::cout << "Age stall" << std::endl;
        }
        
        //if (!p->pipe_latch[ID_LATCH][i].stall || p->pipe_latch[ID_LATCH][i].op_id < 3 ) {
...
void pipe_cycle_IF(Pipeline *p)
{
    for (unsigned int i = 0; i < PIPE_WIDTH; i++)
    {
        if (p->pipe_latch[ID_LATCH][i].op_id == p->pipe_latch[IF_LATCH][i].op_id) {
