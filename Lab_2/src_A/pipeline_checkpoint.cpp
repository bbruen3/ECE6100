...
void pipe_cycle_ID(Pipeline *p)
{
    ...
    for (unsigned int i = 0; i < PIPE_WIDTH; i++)
    {
        ...
        if (ENABLE_EXE_FWD)
        {
            ...
            std::cout << "Before stall_src2_ex " << stall_src2_ex << std::endl;
            ...
            if (stall_src2_ex) {
                ...
                if (p->pipe_latch[ID_LATCH][0].op_id == 4689) {
                    std::cout << "IN stall_src2_ex 4689 pipe " << i << std::endl;
                    std::cout << "youngest: " << youngest << std::endl;
                    std::cout << stall_src2_ex << std::endl; 
                }
                if (youngest != 256) {
                    bool ld = false;
                    if (p->pipe_latch[EX_LATCH][youngest].trace_rec.op_type==OP_LD) {
                    ...
                    std::cout << "Before reset " << ld << std::endl; 
                    if (p->pipe_latch[EX_LATCH][youngest].trace_rec.op_type!=OP_LD) {
                ...
                if (p->pipe_latch[EX_LATCH][0].op_id == 4689) {
                    std::cout << stall_src2_ex << std::endl; 
                }
            }
